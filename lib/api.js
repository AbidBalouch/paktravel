...
export async function getDestinationsData() {
  ...
  const mapped = terms.map((term) => {
    ...
    return {
      ...
      // Headless frontend route hai /provinces/[provinceSlug] — is liye
      // yahan bhi wahi prefix use karo, WordPress ka URL nahi.
      slug: term.slug,
  ...
  return {
    ...
    items: posts.map((post) => {
      ...
      return {
        ...
        // Headless frontend route hai /attractions/[slug] — WordPress ka
        // "post.link" (backend permalink) yahan use nahi karna.
